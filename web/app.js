...
function kiyohara(s) {
  ...
  const source = s.source_doc_id
    ...
    : "";
  return `<section class="kiyohara"><div class="kHead"><div><small>独自評価モデル</small><h4>清原式・推定評価</h4></div><div class="score"><b>${score}</b><span>/100</span></div></div><div class="verdictRow"><strong class="verdict v${score >= 75 ? 4 : score >= 55 ? 3 : score >= 35 ? 2 : 1}">${esc(s.kiyohara_verdict || "判定準備中")}</strong><span>${doc ? "有価証券報告書を反映" : "数値ベースの暫定評価"}</span></div><p class="kSummary">${esc(s.kiyohara_summary || "次回の更新で推定評価を生成します。")}</p><div class="evidence"><div><b>評価した材料</b><ul>${points(s.kiyohara_positives, "positive")}</ul></div><div><b>注意・反対材料</b><ul>${points(s.kiyohara_cautions, "caution")}</ul></div></div>${source}<p class="estimateNote">公開情報から機械的に推定した調査支援情報であり、清原達郎氏本人の判断・投資助言ではありません。</p></section>`;
}
function tenbagger(s) {
  const score = s.tenbagger_score ?? 0;
  return `<section class="tenbagger"><div class="kHead"><div><small>調査優先度</small><h4>テンバガー候補評価</h4></div><div class="score"><b>${score}</b><span>/100</span></div></div><div class="verdictRow"><strong class="typeBadge type-${esc(s.tenbagger_type || "継続観察")}">${esc(s.tenbagger_type || "継続観察")}</strong><span>${esc(s.tenbagger_verdict || "更新待ち")}</span></div><div class="growthMetrics">${metric("売上高 3年平均成長率", pct(s.revenue_cagr_3y), s.revenue_cagr_3y >= 0.1)}${metric("営業利益 3年平均成長率", pct(s.operating_income_cagr_3y), s.operating_income_cagr_3y >= 0.1)}${metric("四半期売上 前年同期比", pct(s.quarterly_revenue_growth), s.quarterly_revenue_growth >= 0.1)}${metric("営業利益率", pct(s.operating_margin), s.operating_margin >= 0.1)}${metric("利益率の前年差", s.operating_margin_change == null ? "—" : `${s.operating_margin_change >= 0 ? "+" : ""}${pct(s.operating_margin_change)}`, s.operating_margin_change >= 0.02)}${metric("52週高値から", pct(s.distance_from_52w_high), s.distance_from_52w_high >= -0.15)}</div><div class="evidence"><div><b>候補になった理由</b><ul>${points(s.tenbagger_reasons, "positive")}</ul></div><div><b>先に確認するリスク</b><ul>${points(s.tenbagger_risks, "caution")}</ul></div></div><p class="estimateNote">将来の株価上昇を予測・保証する点数ではなく、追加調査の優先順位です。</p></section>`;
}
function catalysts(s) {
  ...
  return `<section class="catalyst"><div class="kHead"><div><small>第2段階／変化・触媒分析</small><h4>変化・触媒モニター</h4></div><div class="score"><b>${score}</b><span>/100</span></div></div><div class="verdictRow"><strong class="signal signal-${score >= 70 ? "strong" : score >= 50 ? "watch" : "neutral"}">${esc(s.revision_signal || "更新待ち")}</strong><span>${esc(s.transformation_signal || "変化を観察")}</span></div>${s.sector || s.industry ? `<p class="businessLine">${esc(classification(s))}</p>` : ""}<div class="growthMetrics">${metric("四半期営業利益 前年同期比", operatingGrowth, s.operating_profit_turnaround || s.quarterly_operating_income_growth >= 0.1)}${metric("売上成長の加速度", pct(s.revenue_acceleration), s.revenue_acceleration >= 0.05)}${metric("利益成長の加速度", pct(s.operating_income_acceleration), s.operating_income_acceleration >= 0.1)}${metric("予想1株利益（EPS）成長率", pct(epsGrowth), epsGrowth >= 0.1)}${metric("出来高／20日平均", s.volume_ratio_20d == null ? "—" : `${num(s.volume_ratio_20d)}倍`, s.volume_ratio_20d >= 1.5)}${metric("52週騰落率", pct(s.return_52w), s.return_52w > 0)}</div><div class="evidence"><div><b>検出した変化</b><ul>${points(s.catalyst_signals, "positive")}</ul></div><div><b>決算・IRで確認する点</b><ul>${points(s.catalyst_checks, "caution")}</ul></div></div><div class="researchLinks"><span>一次情報を確認</span>${links}</div><p class="estimateNote">「上方修正兆候」は公表済みの業績修正を示すものではありません。決算数値と予想値から追加確認の優先度を機械判定しています。</p></section>`;
}
