(function () {
  ...
  function build(stock, saved = {}) {
    ...
    const business = stock.business_summary
      ? `<p>${h(stock.business_summary)}</p>`
      : "<p>事業概要データは未取得です。会社IRで事業構成と収益源を確認してください。</p>";
    ...
    return `<!doctype html><html lang="ja"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>${h(stock.code)} ${h(stock.name)} 詳細投資判断レポート</title><style>
      ...
      <header class="cover"><small>DETAILED INVESTMENT RESEARCH / ${h(generated)}</small><h1>${h(stock.code)}　${h(stock.name)}</h1><p>${h(stock.market || "")}　${h([stock.sector,stock.industry].filter(Boolean).join(" / "))}</p><div class="decision"><span class="pill">最終機械判定 <b>${h(stock.investment_decision || "データ補完を優先")}</b></span><span class="pill">総合 <b class="score">${stock.investment_score ?? 0}</b>/100</span><span class="pill">候補 ${stock.tenbagger_score ?? 0}</span><span class="pill">触媒 ${stock.catalyst_score ?? 0}</span><span class="pill">リスク ${stock.risk_score ?? 0}</span><span class="pill">データ信頼度 ${stock.data_quality_score ?? 0}%</span></div></header>
      <section><p class="eyebrow">EXECUTIVE SUMMARY</p><h2>判断要約</h2><div class="two"><div class="box"><h3>投資仮説を支える材料</h3>${list([...(stock.tenbagger_reasons||[]),...(stock.catalyst_signals||[]),...(stock.risk_protections||[])].slice(0,9))}</div><div class="box"><h3>反証・撤退条件として確認する材料</h3>${list([...(stock.tenbagger_risks||[]),...(stock.catalyst_checks||[]),...(stock.risk_reasons||[])].slice(0,9))}</div></div></section>
      <section><p class="eyebrow">BUSINESS & FUNDAMENTALS</p><h2>事業・ファンダメンタル分析</h2>${business}<div class="grid">
        ${[["株価",money(stock.price)],["時価総額",oku(stock.market_cap)],["PER",stock.per==null?"—":`${number(stock.per)}倍`],["PBR",stock.pbr==null?"—":`${number(stock.pbr)}倍`],["ネットキャッシュ比率",number(stock.net_cash_ratio)],["フリーCF",oku(stock.free_cash_flow)],["ROE",percent(stock.return_on_equity)],["ROA",percent(stock.return_on_assets)],["粗利率",percent(stock.gross_margin)],["負債資本倍率",stock.debt_to_equity==null?"—":`${number(stock.debt_to_equity)}%`],["配当利回り",percent(stock.dividend_yield)],["営業CF",oku(stock.operating_cash_flow)]].map(([a,b])=>`<div class="metric"><span>${a}</span><b>${b}</b></div>`).join("")}
      </div><div class="grid">
        ${[["売上3年CAGR",percent(stock.revenue_cagr_3y)],["営業利益3年CAGR",percent(stock.operating_income_cagr_3y)],["四半期売上YoY",percent(stock.quarterly_revenue_growth)],["四半期営業利益YoY",stock.operating_profit_turnaround?"黒字転換":percent(stock.quarterly_operating_income_growth)],["営業利益率",percent(stock.operating_margin)],["予想EPS成長",percent(stock.forward_eps_growth??stock.earnings_growth)]].map(([a,b])=>`<div class="metric"><span>${a}</span><b>${b}</b></div>`).join("")}
      </div></section>
      <section class="pageBreak"><p class="eyebrow">TECHNICAL ANALYSIS</p><h2>52週チャート・需給分析</h2>${sparkline(stock.price_history_52w)}<div class="grid">
        ${[["テクニカル判定",`${h(stock.technical_trend||"未判定")}・${stock.technical_score??0}点`],["20日移動平均",money(stock.ma20)],["50日移動平均",money(stock.ma50)],["200日移動平均",money(stock.ma200)],["RSI 14日",number(stock.rsi14,1)],["60日年率ボラ",percent(stock.volatility_60d)],["1年最大DD",percent(stock.max_drawdown_1y)],["52週騰落率",percent(stock.return_52w)],["出来高比率",stock.volume_ratio_20d==null?"—":`${number(stock.volume_ratio_20d)}倍`]].map(([a,b])=>`<div class="metric"><span>${a}</span><b>${b}</b></div>`).join("")}
      ...
      <section><p class="eyebrow">SCENARIO ANALYSIS</p><h2>3年シナリオ</h2><table><thead><tr><th>ケース</th><th>EPS成長率</th><th>出口PER</th><th>想定株価</th><th>現在値比</th><th>年率</th></tr></thead><tbody>${scenario(stock,"弱気",Math.max(-.2,baseGrowth-.10),Math.max(6,basePer-4))}${scenario(stock,"基本",baseGrowth,basePer)}${scenario(stock,"強気",Math.min(.4,baseGrowth+.10),Math.min(30,basePer+5))}</tbody></table><p class="disclaimer">EPSが一定率で成長し、指定PERで評価される単純な感応度分析です。業績・株価を予測するものではありません。</p></section>
      <section><p class="eyebrow">PRIMARY SOURCES & DUE DILIGENCE</p><h2>IR一次情報と確認事項</h2><div class="two"><div class="box sources">${company?`<a href="${company}" target="_blank">企業公式サイト</a>`:""}<a href="${h(yahoo)}" target="_blank">開示情報一覧</a><a href="${tdnet}" target="_blank">TDnet 適時開示</a><a href="${edinet}" target="_blank">EDINET 開示書類</a><p>${sourceDescription}</p></div><div class="box"><h3>最終確認チェックリスト</h3>${list(["最新決算短信と会社計画の前提","セグメント別の売上・利益構成","上方修正・下方修正と一過性要因","大株主・希薄化・資本配分","競争優位性と市場規模","投資仮説が崩れる条件と許容損失"])}</div></div></section>
      <section><p class="eyebrow">INVESTMENT JOURNAL</p><h2>利用者メモ</h2><div class="note"><b>確信度：${h(saved.conviction||"未設定")}</b><p>${h(saved.note||"メモ未入力")}</p></div></section>
